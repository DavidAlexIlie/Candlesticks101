...
//usestates
  ...
  const getThemeColors = (colorTheme) => {
    const themes = {
      ...
      "rose-pink": {
        // Backgrounds & Containers  
        quickSummary: "from-rose-800 to-rose-900",
        quickSummaryBorder: "border-rose-600",
        expandable: "from-rose-700 to-rose-800 hover:from-rose-600 hover:to-rose-700",
        expandableBorder: "border-rose-500",
    ...
    if (colorTheme === "teal-green") {
      return {
        ...
        inlineStyles: {
          quickSummary: { background: 'linear-gradient(to right, #0f4f4f, #0a3a3a)' },
          expandable: { background: 'linear-gradient(to right, #0f5f5f, #0f4f4f)' },
          header: { color: '#2dd4bf' },
          title: { color: '#5eead4' },
...
const CandlestickChart2 = ({ candlestickData, title, colorTheme, width = 400, height = 200, themeColors }) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setAnimationStep(prev => (prev + 1) % totalSteps);
    }, 1500);
    ...
    return (
      <div className="bg-gray-800 rounded-lg p-4 border border-gray-600">
        ...
        <svg width={width} height={height} className="mx-auto">
          ...
          <g className="text-xs">
            <text x={width - 40} y={getY(globalHigh) + 5} textAnchor="middle" fill="#d1d5db">${globalHigh.toFixed(0)}</text>
            <text x={width - 40} y={getY(globalLow) + 5} textAnchor="middle" fill="#d1d5db">${globalLow.toFixed(0)}</text>
          </g>
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 border border-gray-600">
      ...
      <svg width={width} height={height} className="mx-auto">
        ...
        <g className="text-xs">
          <text x={width - 40} y={getY(candleData.high) + 5} textAnchor="middle" fill="#d1d5db">${candleData.high}</text>
          <text x={width - 40} y={getY(candleData.low) + 5} textAnchor="middle" fill="#d1d5db">${candleData.low}</text>
        </g>
          ...
          case 'h3':
                ...
                <div key={index} className="bg-blue-900 border-l-4 border-blue-400 p-4 rounded-r-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className={themeColors.subtitleColor}>{item.content}</p>
                    </div>
                  </div>
                </div>
              );
            ...
            case 'quote':
              ...
              case 'expandable':
                ...
                const expandableThemeColors = getThemeColors(currentLesson.colorTheme);
                
                return (
                  <div key={index} className={`border-2 ${expandableThemeColors.expandableBorder} rounded-lg overflow-hidden my-4 shadow-lg`}>
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                      ...
                      className={`w-full p-4 bg-gradient-to-r ${expandableThemeColors.expandable} transition-all duration-200 flex items-center justify-between text-left transform hover:scale-[1.02]`}
                    >
                    ...
                    {isExpanded && (
                      <div className="p-6 bg-gray-800 space-y-4 border-t border-purple-400">
